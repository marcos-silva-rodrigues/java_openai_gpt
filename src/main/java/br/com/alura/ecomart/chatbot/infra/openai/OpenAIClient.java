...
public class OpenAIClient {
  ...
  public OpenAIClient(@Value("${app.openai.api.key}") String apiKey, @Value("${app.openai.assistent.id}") String assistentId, CalculadorDeFrete calculadorDeFrete) {
    this.apiKey = apiKey;
  ...
  public String enviarRequisicaoChatCompletion(DadosRequisicaoChatCompletion dados) {
    ...
    if (threadId == null) {
      var threadRequest = ThreadRequest.builder().messages(Collections.singletonList(messageRequest)).build();
    ...
    var runRequest = RunCreateRequest.builder().assistantId(assistentId).build();
  ...
  public List<String> carregarHistoricoDeMensagens() {
    ...
    if (this.threadId != null) {
      mensagens.addAll(service.listMessages(this.threadId).getData().stream().sorted(Comparator.comparingInt(Message::getCreatedAt)).map(m -> m.getContent().get(0).getText().getValue()).collect(Collectors.toList()));
    }
  ...
  private String chamarFuncao(Run run) {
    try {
      ...
      var funcaoCalcularFrete = ChatFunction.builder().name("calcularFrete").executor(DadosCalculoFrete.class, d -> calculadorDeFrete.calcular(d)).build();
